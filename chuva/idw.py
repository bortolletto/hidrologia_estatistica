...
base = xr.open_dataset("../../lf_pm/catch/maps/outlets.nc").to_dataframe()
base_temp   = xr.open_dataset("../../lf_pm/catch/meteo/chuvas/pr.nc")
...
for ano in dados.index.year.unique():
    ...
    temp = dados[selected_stations].loc[str(ano)]
    temp_loc = locais.loc[locais.codigo.isin([int(x) for x in selected_stations]) == True].copy()
